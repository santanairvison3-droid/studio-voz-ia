...
module.exports = async (req, res) => {
  ...
  // ── USERS ──
  if (req.method === 'GET') {
    const { data, error } = await supabase.from('users').select('id, username, name, email, role, plan, status, created_at').order('created_at', { ascending: false });
    if (error) return res.status(500).json({ error: error.message });
    return res.status(200).json(data);
  }
  ...
  if (req.method === 'POST') {
    ...
    if (action === 'create') {
      const { data, error } = await supabase.from('users').insert({ username, name, email, pw, role: role || 'user', plan: plan || 'basico', status: status || 'ativo' }).select().single();
      if (error) return res.status(500).json({ error: error.message });
      return res.status(201).json(data);
    }
