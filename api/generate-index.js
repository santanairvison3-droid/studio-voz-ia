...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    // Verifica limite diário (silencioso)
    let usedToday = 0;
    try {
    ...
    try {
      const r = await fetch('https://app.darkplanner.com.br/api/v1/audio/generate', {
        ...
        // DarkPlanner usa "id" como campo da voz, não "voice_id"
        body: JSON.stringify({ text, id: voice_id, voice_id })
      });
      ...
      data.audio_url = data.audio_url || data.url || data.download_url || data.file_url || data.audio || null;
      data.job_id    = data.job_id    || data.task_id || null;
